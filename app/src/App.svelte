<script lang="ts">
	...
	$: selectedWallpaper = selectedFolderName
		? { ...wallpapers[selectedFolderName], folderName: selectedFolderName }
		: null;
	$: activeWallpaper = activeFolderName ? { ...wallpapers[activeFolderName], folderName: activeFolderName } : null;
	...
	async function initialize() {
		...
		for (const folderName in wallpapers) {
			...
			if (wallpaperData.previewPath) {
				const previewResult = await wallpaperManager.getWallpaperPreview(wallpaperData.previewPath);
				if (previewResult.success) {
					wallpapers[folderName] = { ...wallpapers[folderName], previewData: previewResult.data };
				} else {
					console.error(`Failed to get preview for ${folderName}: ${previewResult.error}`);
				}
			}
		}
	}
	...
	async function handleSelectWallpaper(event: CustomEvent) {
		...
		if (selectedScreenForConfig) {
			await wallpaperManager.setWallpaper(selectedScreenForConfig, folderName);
...
<div class="app-container">
	...
	<div class="content">
		<WallpaperGrid {wallpapers} {selectedWallpaper} {loading} {error} on:select={handleSelectWallpaper} />
		<Sidebar {selectedWallpaper} on:close={() => (selectedFolderName = null)} />
	</div>
