...
func stageFlatpakHelper() (string, string, error) {
	...
	// --persist=. makes Steam's Flatpak app-data directory appear as its HOME.
	insidePath := "$HOME/.local/share/linux-wallpaperengine-gui/" + name
	return hostPath, insidePath, nil
}
...
func startFlatpakHelper() (*flatpakHelperClient, error) {
	...
	_, insideHelper, err := stageFlatpakHelper()
	if err != nil {
		return nil, err
	}
	...
	cmd := exec.Command("flatpak", "enter", flatpakSteamAppID, "/bin/sh", "-lc", command)
...
func getFlatpakClient() (*flatpakHelperClient, error) {
	...
	if flatpakClient != nil && flatpakClient.cmd.ProcessState == nil {
